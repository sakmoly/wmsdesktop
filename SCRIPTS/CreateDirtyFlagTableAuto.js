// Auto-create dirty flag table using provided credentials
...
async function createDirtyFlagTable() {
  ...
  try {
    ...
    console.log('----------------');
    columns.forEach(col => {
      console.log(`  ${col.COLUMN_NAME.padEnd(20)} ${col.DATA_TYPE.padEnd(15)} ${col.IS_NULLABLE === 'YES' ? 'NULL' : 'NOT NULL'} ${col.COLUMN_DEFAULT ? `DEFAULT ${col.COLUMN_DEFAULT}` : ''}`);
    });
    ...
    console.log('--------');
    indexes.forEach(idx => {
      console.log(`  ${idx.INDEX_NAME.padEnd(20)} ${idx.COLUMN_NAME.padEnd(20)} ${idx.NON_UNIQUE === 0 ? 'UNIQUE' : 'NON-UNIQUE'}`);
    });
