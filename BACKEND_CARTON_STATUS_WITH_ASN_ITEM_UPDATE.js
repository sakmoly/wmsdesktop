...
// UPDATED: Now also updates tabAsnItemDetails.carton_assigned_status when updating carton status
 ...
 * - NEW: Also updates tabAsnItemDetails.carton_assigned_status
 * 
...
export const updateCartonStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Helper function to update tabAsnItemDetails
    const updateAsnItemDetails = async (cartonId, cartonStatus, asn) => {
      try {
        await connection.query(`
          ...
          WHERE carton_id = ? 
            AND parent_title = ?
        `, [cartonStatus, cartonId, asn]);
        logger.info({ carton_id: cartonId, asn_no: asn, status: cartonStatus }, 'Updated tabAsnItemDetails.carton_assigned_status');
      } catch (asnItemError) {
        // Non-critical - log but don't fail
        logger.warn({ error: asnItemError.message, carton_id: cartonId, asn_no: asn }, 'Failed to update tabAsnItemDetails (non-critical)');
      }
    };
    ...
    if (isSingle) {
      ...
      if (updateResult.affectedRows > 0) {
        ...
        await updateAsnItemDetails(carton_id, status, exactAsn);
      } else {
      ...
      for (const carton of cartons) {
        ...
        if (updateResult.affectedRows > 0) {
          ...
          // Also update tabAsnItemDetails.carton_assigned_status
          await updateAsnItemDetails(carton.carton_id, carton.status, exactAsn);
        } else {
          // Carton doesn't exist - INSERT it (UPSERT logic)
          try {
            await connection.query(
